...
local Window = Fluent:CreateWindow({
    ...
    SubTitle = "Overhaul (2.4 Version) Made by Unknownproooolucky",
    TabWidth = 160,
...
-- Переменные для хранения состояний и соединений
local unlockedPassLoop = nil
local exchangeOpenLoop = nil

-- Первый тумблер: Unlocked Pass (скрывает элемент "Unlocked")
local UnlockedPassToggle = EventTab:AddToggle("UnlockedPassToggle", {
    Title = "Hide 'Unlocked' Text",
    Description = "Скрывает надпись 'Unlocked' в Battlepass",
    Default = false,
    Callback = function(state)
        if state then
            -- Включаем скрытие
            if not unlockedPassLoop then
                unlockedPassLoop = RunService.Heartbeat:Connect(function()
                    pcall(function()
                        -- Путь к элементу, который нужно скрыть
                        local unlockedElement = game:GetService("Players").LocalPlayer.PlayerGui.Menu.Views.Battlepass.ViewPass.Center.ViewPass.Unlocked
                        if unlockedElement then
                            unlockedElement.Visible = false
                        end
                    end)
                end)
            end
            ...
            -- Отключаем скрытие
            if unlockedPassLoop then
                unlockedPassLoop:Disconnect()
                unlockedPassLoop = nil

                -- Восстанавливаем видимость элемента
                pcall(function()
                    local unlockedElement = game:GetService("Players").LocalPlayer.PlayerGui.Menu.Views.Battlepass.ViewPass.Center.ViewPass.Unlocked
                    if unlockedElement then
                        unlockedElement.Visible = true
                    end
                end)
            end
        end
...
-- Второй тумблер: Exchange Open (переключает на вкладку Exchange)
local ExchangeOpenToggle = EventTab:AddToggle("ExchangeOpenToggle", {
    Title = "Force Open Exchange Tab",
    Description = "Принудительно открывает вкладку Exchange",
    Default = false,
    Callback = function(state)
        if state then
            -- Включаем принудительное открытие Exchange
            if not exchangeOpenLoop then
                exchangeOpenLoop = RunService.Heartbeat:Connect(function()
                    pcall(function()
                        local battlepassView = game:GetService("Players").LocalPlayer.PlayerGui.Menu.Views.Battlepass

                        -- Показываем Exchange
                        if battlepassView.Exchange then
                            battlepassView.Exchange.Visible = true
                        end

                        -- Скрываем Center (старый ViewPass)
                        if battlepassView.Center then
                            battlepassView.Center.Visible = false
                        end

                        -- Скрываем ViewPass на всякий случай
                        if battlepassView.ViewPass then
                            battlepassView.ViewPass.Visible = false
                        end
                    end)
                end)
            end
        else
            -- Отключаем и возвращаем все как было
            if exchangeOpenLoop then
                exchangeOpenLoop:Disconnect()
                exchangeOpenLoop = nil
                ...
                pcall(function()
                    ...
                    -- Скрываем Exchange
                    if battlepassView.Exchange then
                        battlepassView.Exchange.Visible = false
                    end

                    -- Показываем Center (ViewPass)
                    if battlepassView.Center then
                        battlepassView.Center.Visible = true
                    end

                    -- Показываем ViewPass (если он был скрыт)
                    if battlepassView.ViewPass then
                        battlepassView.ViewPass.Visible = true
                    end
                end)
            end
        end
...
EventTab:AddParagraph({
    Title = "Примечание",
    Content = "Эти функции изменяют отображение интерфейса Battlepass. Отключите их перед закрытием меню."
})
